...
vec4 computLightObject(vec4 objectColor, vec3 normal, vec3 fragPos)
{
	...
	// 环境光直接根据光照颜色和环境光强度得到
	vec3 ambientColor = light.strength.x * light.ambient; 
	...
	vec3 diffuseColor = light.strength.y*diff*light.diffuse;
	...
	vec3 specularColor = light.strength.z * spec * light.specular;
