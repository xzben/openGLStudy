...
{
	NEAREST, //临近过滤  选择中心点最接近纹理坐标的那个像素  只用于 放大线性过滤
	LINEAR, // 线性过滤  它会基于纹理坐标附近的纹理像素，计算出一个插值，近似出这些纹理像素之间的颜色 只用于 放大线性过滤

	NEAREST_MIPMAP_NEAREST,  // 使用最邻近的多级渐远纹理来匹配像素大小，并使用邻近插值进行纹理采样  只用于缩小的多级渐远线性过滤
	LINEAR_MIPMAP_NEAREST,   // 使用最邻近的多级渐远纹理级别，并使用线性插值进行采样   只用于缩小的多级渐远线性过滤
	NEAREST_MIPMAP_LINEAR,	// 在两个最匹配像素大小的多级渐远纹理之间进行线性插值，使用邻近插值进行采样  只用于缩小的多级渐远线性过滤
	LINEAR_MIPMAP_LINEAR,   // 在两个邻近的多级渐远纹理之间使用线性插值，并使用线性插值进行采样 只用于缩小的多级渐远线性过滤
};
